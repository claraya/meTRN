...
def main():
	...
	# import mode:
		...
		# Tissue labels are organised from specific-to-generic as follows:
		# specificDict:  cell > specific tissue
		# generalDict: specific tissue > general tissue
		# classDict:   general tissue > class tissue (tissue class)
		
		# construct tissue dictionary (if necessary):
		if option.tissues != "OFF" and option.mapping != "OFF":
			
			print
			print "Loading general and specific tissue information..."
			specificDict, specificTotal, specificMissing = dict(), list(), 0
			generalDict, generalTotal, generalMissing = dict(), list(), 0
			tslines = open(extraspath + option.tissues).read().replace("\r","\n").split("\n")
			inlines = open(extraspath + option.mapping).read().replace("\r","\n").split("\n")
			specificHeader = inlines.pop(0)
			for inline in inlines:
				if inline.strip() != "":
					regExp, generalTissue = inline.strip().split(",")
					generalTissue = generalTissue.strip().lower()
					specificHits = list()
					for tsline in tslines:
						if tsline.strip() != "":
							cell, specificTissue = tsline.strip().split(", ")
							if re.match(regExp, specificTissue):
								specificTissue = specificTissue.strip().lower()
								specificDict[cell] = specificTissue
								generalDict[cell] = generalTissue
								#print " : ".join([cell, specificTissue, generalTissue, regExp])
								#pdb.set_trace()
								specificTotal.append(specificTissue)
								generalTotal.append(generalTissue)
			specificTotal = sorted(list(set(specificTotal)))	
			generalTotal = sorted(list(set(generalTotal)))	
			
			print "Generating tissue classes..."
			classification = {
				"neuron" : "neuron/glial",
				"pharynx" : "pharynx",
				"intestine" : "intestine/excretory",
				"intesto" : "intestine/excretory",
				"execretory" : "intestine/excretory"
				}
		...
		if option.tissues != "OFF" and option.mapping != "OFF":
			...
			for cell in pedigreeCells:
				multipleDict[cell] = ""
				if not cell in specificDict:
	
					# Find most closely-related, annotated cell (and use its associated tissue annotation):
					minQuery, queryDict, queryCells, matchCells, matchTissues = 100, dict(), list(), list(), list()
					for queryCell in specificDict:
						if cell == queryCell[:len(cell)] or queryCell == cell[:len(queryCell)]:
							queryCells.append(queryCell)
					for queryCell in queryCells:
						if len(queryCell) < minQuery:
							minQuery = len(queryCell)
					for queryCell in queryCells:
						if len(queryCell) == minQuery:
							matchCells.append(queryCell)
							queryTissue = specificDict[queryCell]
							if not queryTissue in matchTissues:
								matchTissues.append(queryTissue)
					
					matchCounts = dict()
					for matchTissue in matchTissues:
						matchCounts[matchTissue] = matchTissues.count(matchTissue)
					matchTissues = general.valuesort(matchCounts)
					...
					specificDict[cell] = matchTissue
					
					
					# Store matches for general and class tissues:
					if len(matchCells) == 0:
						generalDict[cell] = "other"
						classDict[cell] = "other"
					else:
						matchCell = matchCells[0]
						generalDict[cell] = generalDict[matchCell]
						classDict[cell] = classDict[matchCell]
			...
			print "Specific tissues missed (converted to other):", matchMissing, "(total:", matchTotal, ")"
			print "General tissues missed (converted to other):", generalMissing, "(total:", len(generalTotal), ")"
			#pdb.set_trace()
		...
		for gene in gene2cell_dict:
			for cell in gene2cell_list[gene]:
				values, maxSeries, maxValue = list(), "", 0
		...
		expressionHeader = ["cell", "cell.name", "gene", "cell.expression", "peak.expression", "fraction.expression", "normal.expression", "rank", "max.expression", "avg.expression", "med.expression", "std.expression", "cells.expressing", "cells.count", "series.count", "time.series", "max.series", "multiple.tissue", "specific.tissue", "general.tissue", "class.tissue"]
		...
		tissueHeader = ["cell","multiple.tissue", "specific.tissue", "general.tissue", "class.tissue"]
		...
		for gene in sorted(matrix.keys()):
			...
			for cell in sorted(matrix[gene].keys()):
				if option.tissues == "OFF" or not cell in specificDict:
					...
					multipleTissue = multipleDict[cell]
					specificTissue = specificDict[cell]
					generalTissue = generalDict[cell]
					classTissue = classDict[cell]
				cellValue, peakValue, fracValue, cellRank, maxValue, meanValue, medianValue, stdValue, seriesCount, seriesIDs, maxSeries = matrix[gene][cell]
				print >>a_output, "\t".join(map(str, [cell, cell, gene, cellValue, peakValue, fracValue, float(cellValue)/maxAssayed[gene], cellRank, maxValue, meanValue, medianValue, stdValue, cellsExpressingAssayed, cellsAssayed, seriesCount, seriesIDs, maxSeries, multipleTissue, specificTissue, generalTissue, classTissue]))
				if cell in startedCells:
					print >>s_output, "\t".join(map(str, [cell, cell, gene, cellValue, peakValue, fracValue, float(cellValue)/maxStarted[gene], cellRank, maxValue, meanValue, medianValue, stdValue, cellsExpressingTracked, cellsStarted, seriesCount, seriesIDs, maxSeries, multipleTissue, specificTissue, generalTissue, classTissue]))
				if cell in trackedCells:
					print >>t_output, "\t".join(map(str, [cell, cell, gene, cellValue, peakValue, fracValue, float(cellValue)/maxTracked[gene], cellRank, maxValue, meanValue, medianValue, stdValue, cellsExpressingTracked, cellsTracked, seriesCount, seriesIDs, maxSeries, multipleTissue, specificTissue, generalTissue, classTissue]))
				if cell in focusedCells:
					print >>f_output, "\t".join(map(str, [cell, cell, gene, cellValue, peakValue, fracValue, float(cellValue)/maxFocused[gene], cellRank, maxValue, meanValue, medianValue, stdValue, cellsExpressingFocused, cellsFocused, seriesCount, seriesIDs, maxSeries, multipleTissue, specificTissue, generalTissue, classTissue]))
				if fracValue >= option.fraction and cellValue >= option.minimum:
		...
		for cell in sorted(specificDict.keys()):
			...
			generalTissue = generalDict[cell]
			classTissue = classDict[cell]
			print >>x_output, "\t".join([cell, multipleTissue, specificTissue, generalTissue, classTissue])
		...
		for gene in sorted(assayedMatrix.keys()):
			...
			for inheritCell in inheritCells:
				...
				if ascendantExpression != dict():
					...
					inheritExpression[gene][inheritCell]["multiple.tissue"] = tissuesAnnotation[inheritCell]["multiple.tissue"]
					...
					maximalExpression[gene][inheritCell]["multiple.tissue"] = tissuesAnnotation[inheritCell]["multiple.tissue"]
	...
	# annotate tissue composition in neurons:
		...
		annotationDict = general.build2(expressionpath + option.expression, id_column="cell", value_columns=["specific.tissue", "general.tissue", "class.tissue"], skip=True, verbose=False)
		cell = annotationDict.keys()[0]
		...
		specificTissues, generalTissues, classTissues = list(), list(), list()
		...
		cellList = list()
		cellMatrix, specificMatrix, generalMatrix, classMatrix = dict(), dict(), dict(), dict()
		for neuron in neuronDict:
			...
			for cell in neuronDict[neuron]["class.ids"].split(","):
				specificTissue, generalTissue, classTissue = annotationDict[cell]["specific.tissue"], annotationDict[cell]["general.tissue"], annotationDict[cell]["class.tissue"]
		...
		specificTallies, generalTallies, classTallies = dict(), dict(), dict()
		...
		# fraction.ids: fraction, match ID/IDs found in neuron.
		...
		print
		specificMissed, generalMissed, classMissed = set(specificTissues).difference(set(specificTallies.keys())), set(generalTissues).difference(set(generalTallies.keys())), set(classTissues).difference(set(classTallies.keys()))
...
#python mapCells.py --path ~/ceTRN --mode import --infile murray_2012_supplemental_dataset_1_per_gene.txt --name murray # Retired!

#python mapCells.py --path ~/ceTRN --mode import --infile waterston_avgExpression.csv --name waterston --measure max.expression
#python mapCells.py --path ~/ceTRN --mode import --infile waterston_avgExpression.csv --name waterston --measure avg.expression

#python mapCells.py --path ~/ceTRN --mode check.status --peaks optimal_standard_factor_sx_rawraw --name waterston --measure avg.expression
#python mapCells.py --path ~/ceTRN --mode check.status --peaks optimal_standard_factor_ex_rawraw --name waterston --measure avg.expression

#python mapCells.py --path ~/ceTRN/ --mode build.lineages --pedigree waterston_cell_pedigree.csv --expression mapcells_avgExp_waterston_expression_tracked --name waterston.tracked --method builder --lineages tracked --descendants OFF --ascendants OFF --limit 10000
#python mapCells.py --path ~/ceTRN/ --mode build.lineages --pedigree waterston_cell_pedigree.csv --expression mapcells_avgExp_waterston_expression_tracked --name waterston.tracked --method builder --lineages complete --descendants OFF --ascendants OFF --limit 10000

#python mapCells.py --path ~/ceTRN/ --mode test.lineages --pedigree waterston_cell_pedigree.csv --expression mapcells_avgExp_waterston_expression_tracked --name waterston.tracked --method builder --lineages tracked --descendants OFF --ascendants OFF --limit 10000
#python mapCells.py --path ~/ceTRN/ --mode test.lineages --pedigree waterston_cell_pedigree.csv --expression mapcells_avgExp_waterston_expression_assayed --name waterston.assayed --method builder --lineages tracked --descendants OFF --ascendants OFF --limit 10000